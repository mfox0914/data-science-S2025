...
df_q4 <-
  diamonds %>%
    ...
    summarize(
      # Need na.rm to remove na values
...
-   Comment on the relative values of `cov_depth` and `cov_diff`.
    -   cov_depth is relatively small, which means that the values of depth are all generally very close to each other and therefore the mean. cov_diff is very large, which means that the values of the difference between the depth and the computed depth are all very far away from each other. This makes sense, as if the dataset is correct in its depth calculations it could be assumed that diff would be small for most rows.
...
I've observed that the premium cut of diamond has the highest mean price compared to the others, with fair having the second highest mean price, with very good, good, and then ideal coming last. This is very surprising, as one would assume that cut and price would have a positive correlation, with price going up as the cut improves. However, the data has shown otherwise, with the worst cut of diamond actually having the second highest price.\
\
The value of cov is very large, supporting the hypothesis that cut is not a strong determining factor of price.
