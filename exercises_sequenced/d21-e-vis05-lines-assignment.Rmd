...
The line plot is assuming that each value is numerical and each row has completely different numerical values.

\- How does the `mpg` dataset violate this assumption?

There are rows with different hwy values but the same displ value.
...
-   The raw data is very jagged with large spikes, it does still generally follow the shape of the smoothed data. The raw data shows shorter trends, while the smoothed data shows a longer, overall trend.
