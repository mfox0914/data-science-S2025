...
-   [Grading Rubric](#grading-rubric)
    ...
    -   [Submission](#submission)
        ...
        -   [**q5** You have access to a few other variables. Construct a **at least three** visualizations of `VelocityVacuum` against these other factors. Are there other patterns in the data that might help explain the difference between Michelson’s estimate and `LIGHTSPEED_VACUUM`?](#q5-you-have-access-to-a-few-other-variables-construct-a-at-least-three-visualizations-of-velocityvacuum-against-these-other-factors-are-there-other-patterns-in-the-data-that-might-help-explain-the-difference-between-michelsons-estimate-and-lightspeed_vacuum)
...
Compared to Michelson’s, the computed MeanVelocity seems to be rounded to the tens place, while Michelson’s is rounded to a whole number. However, even if the computed MeanVelocity were rounded to the nearest whole number, the numbers would still be different. A more accurate observation is to say that Michelson’s is rounded to the closest whole number with 5 significant numbers. This suggests that Michelson may have applied additional rounding, such as rounding after computing the mean rather than rounding individual values first, or using some other method of handling precision.
...
**Observations**: - Is Michelson’s estimate of the error (his uncertainty) greater or less than the true error? Michelson’s estimate of his error was less than the actual error, with his error being +- 51 and the real error being +151.542.

-   Make a quantitative comparison between Michelson’s uncertainty and his error.

Michelson’s uncertainty was about 3 times less than his actual error. This shows he was very confident in his answer but might have forgotten to account for a physical difference between his experimental setup and reality.
...
**Observations**: Similarities - The mean of both velocities stays inbetween 299800 and 300100 all days. Both means tend to increase and decrease at around the same times.

Differences - The real data has more extreme outliers. The real data tends to vary more from day to day.
...
df_q2 %>%
  ggplot(aes(x = Distinctness, y = abs(VelocityVacuum - LIGHTSPEED_VACUUM))) +
  geom_point()
```
...
-   The spread of the difference is large for all distinctness values. This implies that distinctness was most likely not the cause of Michelson’s error.

-   The more distinct images actually deviate more from the true value, further supporting the observation that distinctness is not the cause of the error.
...
-   There is no noticeable trend in the graph.
    ...
    df_q2 %>%
      ggplot(aes(x = Temp, y = abs(VelocityVacuum - LIGHTSPEED_VACUUM))) +
      geom_point() +
...
-   While there is a slight positive correlation between temperature and the error in Michelson’s estimate, the data is also very spread out.

-   While the trendline could lead one to think that temperature might have had a small influence on Michelson’s error, this is most likely coincidence due to the large, almost random spread of the data.

-   Lower temperatures tended to have a smaller error than higher temperatures.

    ``` r
    # Difference between velocity and true value of lightspeed v. Date
    df_q2 %>%
      ggplot(aes(x = Date, y = abs(VelocityVacuum - LIGHTSPEED_VACUUM))) +
      geom_point() +
      geom_smooth(method = lm)
    ```
...
-   However, the data is still scattered, implying that this trend may be coincidental.
-   The reason for Michelson’s error does not seem to be related to the date.
