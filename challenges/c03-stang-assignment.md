...
- Is there “one true value” for the material properties of Aluminum?
  - No, because elasticity and Poisson’s ratio vary with angle and
    thickness.
- How many aluminum alloys are in this dataset? How do you know?
  - There is only 1 aluminum alloy in this dataset because when you look
    at all the unique values of each column there is only one value for
    alloy, “al_24st”.
...
- Are there any values that look out of place?
  - There is a value of negative 1 that is in both the E and nu columns.
    This number does not seem to make physical sense based on research
    and differs greatly from the other values in both those columns,
    suggesting this may be an error or a placeholder.
...
- There are 26 rows but only 12 points, suggesting that E values are
  repeated for the same thickness.
- There is noticeable variation in E for the same thickness, which could
...
- Does this graph support or contradict the claim above?
  - The graph is showing that E varies with thickness, though the value
    of E is generally close for the same thickness.
  - This suggests that E is not a purely intensive material property in
    this dataset and therefore contradicts the claim above.
- Is this evidence *conclusive* one way or another?
  - No, because the values are clustered together for the same thickness
    value, it can be inferred that the observed variation in E could be
    caused by measurement errors, differences in how samples were
    prepared, or other external influences.
